// Force GEMINI_API_KEY: Always delete GOOGLE_API_KEY to avoid SDK
confusion

// The SDK prints "Both GOOGLE_API_KEY and GEMINI_API_KEY are set" and
uses the wrong one

if (process.env.GOOGLE_API_KEY) {

console.log("Deleting GOOGLE_API_KEY from environment to force SDK to
...
import { users, courses, progress, tickets, certificates, lessonAudio,
lessonImages } from "../shared/schema";

import { eq, and } from "drizzle-orm";

import { ObjectStorageService, ObjectNotFoundError, objectStorageClient
} from "./objectStorage";
...
// Serve objects from Replit Object Storage (handles paths like
/objects/videos/abc.mp4)
...
const objectFile = await
objectStorageService.getObjectEntityFile(req.path);
...
if (/^[A-Za-z0-9+/=]+$/.test(data.slice(0, 100)) && data.length > 1000)
return true;
...
function saveBase64ToFile(base64Data: string, type: 'images' | 'audio' |
'video', extension: string): string | null {
...
const hash =
crypto.createHash('md5').update(buffer).digest('hex').slice(0, 12);
...
// Object Storage is not used - all media goes to lesson_audio and
lesson_images tables
...
console.log('Object Storage: Testing with PRIVATE_OBJECT_DIR =',
privateDir);

const { bucketName, objectName } = parseObjectPath(privateDir);

console.log('Object Storage: Parsed bucket =', bucketName, ', path =',
objectName);
...
console.log('Object Storage status: NOT CONFIGURED (bucket not in
allowed resources)');

console.log('HINT: Add the bucket to allowed resources in the Object
Storage panel');

} else {

console.log('Object Storage status: NOT AVAILABLE -', err.message ||
String(err));
...
function parseObjectPath(path: string): { bucketName: string;
objectName: string } {
...
async function saveVideoToObjectStorage(base64Data: string):
Promise<string | null> {

if (!isBase64Data(base64Data)) return null;

if (base64Data.startsWith('/objects/') ||
base64Data.startsWith('/media/') || base64Data.startsWith('http')) {
...
const url = await objectStorageService.uploadVideoFromBase64(base64Data,
'video.mp4');
...
async function saveImageToObjectStorage(base64Data: string, extension:
string = 'png'): Promise<string | null> {
...
const url = await objectStorageService.uploadImageFromBase64(base64Data,
`image.${extension}`);
...
async function saveAudioToObjectStorage(base64Data: string):
Promise<string | null> {
...
const url = await objectStorageService.uploadAudioFromBase64(base64Data,
'audio.mp3');
...
// If Object Storage is not working, keep ALL media as base64 in
Supabase database
...
console.log('Object Storage not available - storing all media as base64
...
// Process ecover - use Object Storage if available (already handled
above with local disk)
...
const url = await saveImageToObjectStorage(updatedLesson.imageUrl,
'jpg');
...
const url = await
saveVideoToObjectStorage(updatedLesson.renderedVideoUrl);
...
const url = await saveImageToObjectStorage(updatedVisual.imageData,
'png');
...
console.log(`Gemini TTS request: voice=${modelName}, text
length=${text.length}`);
...
console.log(`Gemini TTS success: audio data
...
return res.status(401).json({ error: "API key issue. Please check server
configuration." });

}

return res.status(500).json({ error: error.message || "TTS generation
failed" });
...
console.log(`ElevenLabs TTS request: voiceId=${voiceId}, text
length=${text.length}`);
...
wordTimestamps.push({ word: currentWord.trim(), start: wordStart, end:
wordEnd });
...
console.log(`ElevenLabs TTS success: ${wordTimestamps.length} word
timestamps`);
...
console.log(`ElevenLabs TTS success (no timestamps): audio
...
return res.status(500).json({ error: error.message || "ElevenLabs TTS
failed" });
...
return res.status(400).json({ error: "Invalid type. Must be image,
audio, or video" });
...
console.log(`Media upload: type=${type}, mimeType=${clientMimeType ||
'auto'}, size=${(dataSize/1024).toFixed(1)}KB`);
...
// ALWAYS store all media as base64 in Supabase database - local disk is
ephemeral

let cleanBase64 = data;

let mimeType = clientMimeType || (type === 'image' ? 'image/png' : type
...
console.log(`Media uploaded: storage=${storage}, url=${url?.substring(0,
50)}...`);
...
return res.status(500).json({ error: error.message || "Media upload
failed" });
...
const results: Array<{ index: number; success: boolean; url?: string;
error?: string }> = [];
...
let mimeType = type === 'image' ? 'image/png' : type === 'audio' ?
...
console.log(`Batch upload complete: ${results.filter(r =>
r.success).length}/${items.length} succeeded`);
...
return res.status(500).json({ error: error.message || "Batch upload
failed" });
...
app.put("/api/courses/:courseId/lessons/:lessonId/takeaways", async
(req, res) => {
...
console.log(`Saving takeaways for course=${courseId},
lesson=${lessonId}`);

// Get the course from database

const courseRows = await db.select().from(courses).where(eq(courses.id,
courseId));
...
app.put("/api/courses/:courseId/lessons/:lessonId/audio", async (req,
res) => {
...
console.log(`Saving audio for course=${courseId}, lesson=${lessonId},
...
console.log(`[Audio Save] Updated audio for lesson ${lessonId}, result:
...
console.log(`[Audio Save] Inserted new audio for lesson ${lessonId},
...
const verifyCount = await db.select({ id: lessonAudio.id
}).from(lessonAudio);

console.log(`[Audio Save] Total records in lesson_audio table:
${verifyCount.length}`);
...
app.get("/api/courses/:courseId/lessons/:lessonId/audio", async (req,
res) => {
...
const dataUrl = `data:${audio.mimeType ||
'audio/mpeg'};base64,${audio.audioData}`;
...
app.get("/api/courses/:courseId/lessons/:lessonId/audio/exists", async
(req, res) => {
...
app.post("/api/courses/:courseId/lessons/:lessonId/images", async (req,
res) => {
...
const { images } = req.body; // Array of { visualIndex, imageData,
prompt }
...
console.log(`Saving ${images.length} images for course=${courseId},
lesson=${lessonId}`);
...
app.get("/api/courses/:courseId/lessons/:lessonId/images", async (req,
res) => {
...
console.log(`[Get Images] Fetching images for course=${courseId},
lesson=${lessonId}`);
...
// Convert to array format with data URL prefix (only if not already
present)
...
console.log(`[Get Images] Returning ${result.length} images, first
visualIndex: ${result[0]?.visualIndex}, response size: ${sizeMB}MB`);
...
// Get image metadata (indices only, no data) for lazy loading - MUST be
before /:visualIndex

app.get("/api/courses/:courseId/lessons/:lessonId/images/metadata",
async (req, res) => {
...
console.log(`[Get Metadata] Fetching for course=${courseId},
lesson=${lessonId}`);
...
app.get("/api/courses/:courseId/lessons/:lessonId/images/exists", async
(req, res) => {
...
// Get a single image by visualIndex (for lazy loading) - MUST be after
/metadata and /exists

app.get("/api/courses/:courseId/lessons/:lessonId/images/:visualIndex",
async (req, res) => {
...
console.log(`[Get Single Image] Fetching index ${visualIndex} for lesson
${lessonId}`);
...
console.log(`[Get Single Image] Found image, size:
${(imageData.length/1024).toFixed(0)}KB`);
...
console.log(`[Fix Timing] Starting for course=${courseId},
lesson=${lessonId}`);
...
console.log(`[Fix Timing] Found ${wordTimestamps.length} word
timestamps`);
...
console.log(`[Fix Timing] Found lesson with ${lesson.visuals.length}
visuals`);

// Step 3: Calculate timing for each visual based on word timestamps

// Strategy: Each visual has scriptText - find matching words and use
their timestamps
...
const remainingTime = (wordTimestamps[wordTimestamps.length - 1]?.end ||
60) - lastEndTime;
...
const lastScriptWord = scriptWords[scriptWords.length -
1]?.toLowerCase();
...
// Check if this word is in the script OR if we're at the next visual's
script
...
const nextScript = (lesson.visuals[i + 1].scriptText ||
"").toLowerCase();

const nextFirstWord = nextScript.split(/\s+/).filter(w => w.length >
2)[0]?.toLowerCase();

if (nextFirstWord && wt.word.toLowerCase().replace(/[^a-z]/g, '') ===
nextFirstWord) {
...
console.log(`[Fix Timing] Visual ${i}: ${startTime.toFixed(1)}s -
${endTime.toFixed(1)}s`);
...
console.log(`[Fix Timing] Complete! Updated ${updatedVisuals.length}
visuals`);
...
timing: updatedVisuals.map((v, i) => ({ index: i, start: v.startTime,
end: v.endTime }))
...
return res.status(400).json({ error: "Missing courseId, lessonId, or
keepCount" });

}

console.log(`[Fix Images] Starting fix for course=${courseId},
lesson=${lessonId}, keeping ${keepCount} images`);
...
console.log(`[Fix Images] Keeping ${imagesToKeep.length} images,
deleting ${imagesToDelete.length}`);
...
console.log(`[Fix Images] Deleted image with visualIndex
${img.visualIndex}`);
...
console.log(`[Fix Images] Complete! Now have ${imagesToKeep.length}
images numbered 0-${imagesToKeep.length - 1}`);
...
message: `Fixed images: deleted ${imagesToDelete.length}, kept and
renumbered ${imagesToKeep.length}`
...
const { name, email, password, avatarUrl, phone, city, state } =
req.body;

const existing = await db.select().from(users).where(eq(users.email,
email));
...
const usersWithoutPasswords = allUsers.map(({ password, ...rest }) =>
rest);
...
console.error(`Get courses error (attempt ${attempt}):`, error?.message
|| error);
...
// Return MINIMAL summaries - courses can be 8MB+ each with embedded
media
...
// For covers: include URL if it's a URL, otherwise flag that cover
exists in DB

const coverUrl = data.ecoverUrl || '';

const hasCoverInDb = coverUrl.startsWith('data:') || coverUrl.length >
1000;
...
description: (data.description || '').substring(0, 500), // Limit
description
...
hasCoverInDb: hasCoverInDb, // Flag to fetch cover from database
endpoint
...
console.log("Returning", coursesData.length, "courses, size:",
...
console.error("Get courses failed after retries:", error?.message ||
error);
...
const [course] = await db.select().from(courses).where(eq(courses.id,
req.params.id));

if (!course) return res.status(404).json({ error: "Course not found" });

res.json({ ...course.data as object, _dbId: course.id });
...
console.log("Creating course with data:", { id: courseData.id, title:
courseData.title });
...
const extractedData = await extractMediaFromCourse({ ...courseData, id:
dbId });
...
console.log(`PUT /api/courses/${req.params.id} - Media extraction
complete`);
...
console.error(`PUT /api/courses/${req.params.id} - Error:`,
error?.message || error);
...
const safeTitle = (courseData.title || 'course').replace(/[^a-z0-9]/gi,
'_');

coursesFolder.file(`${safeTitle}.json`, JSON.stringify(courseData, null,
2));
...
const filename = `all_courses_backup_${new
Date().toISOString().slice(0,10)}.zip`;
...
console.log(`Export complete: ${allCourses.length} courses,
...
modulesFolder.file(`module_${idx}_${mod.id || idx}.json`,
...
console.log("Processing uploaded course ZIP:", req.file.originalname,
"Size:", req.file.size);
...
return res.json({ success: true, courses: savedCourses, count:
savedCourses.length, settings });
...
return res.status(400).json({ error: "Invalid course ZIP: Missing
...
moduleFiles.sort((a, b) => a.path.localeCompare(b.path, undefined, {
numeric: true }));
...
res.json({ success: true, course: { ...newCourse.data as object, _dbId:
newCourse.id } });
...
return res.status(400).json({ error: "Invalid course ZIP: Missing
course_metadata.json or courses.json" });
...
res.status(500).json({ error: error?.message || "Failed to import from
URL" });
...
db.select().from(progress).where(eq(progress.userId,
req.params.userId));
...
.where(and(eq(progress.userId, userId), eq(progress.courseId,
courseId)));
...
results.push({ id: course.id, title: data.title, status: 'failed',
error: err?.message });
...
const filesCreated = countFiles('images') + countFiles('audio') +
countFiles('video');

console.log(`Migration complete: ${migratedCount} courses updated,
${errorCount} failed, ${filesCreated} media files total`);
...
if (isBase64Data(lesson.imageUrl)) { needsMigration = true;
mediaCount++; }

if (isBase64Data(lesson.audioData)) { needsMigration = true;
mediaCount++; }

if (isBase64Data(lesson.renderedVideoUrl)) { needsMigration = true;
mediaCount++; }
...
if (isBase64Data(visual.imageData)) { needsMigration = true;
mediaCount++; }
...
// ONLY use direct Gemini API with GEMINI_API_KEY - NO Replicate, NO
OpenAI
...
return res.status(500).json({ error: 'GEMINI_API_KEY is required for
image generation.' });
...
prompt: "A beautiful sunset over mountains, photorealistic, high
quality",
...
console.log('FLUX test prediction created:', prediction.id, 'status:',
prediction.status);
...
console.log('FLUX test waiting... status:', result.status, 'attempt:',
attempts);
...
console.log('Gemini text gen failed, trying OpenAI fallback:',
geminiError?.message);
...
return res.status(500).json({ error: 'No AI provider available for text
generation' });
...
return res.status(400).json({ error: "Script is required and must be at
least 50 characters" });
...
Keep each item concise (under 100 characters). Focus on practical
value.`;
...
console.log('Gemini takeaways failed, trying OpenAI:',
geminiError?.message);
...
return res.status(500).json({ error: 'Failed to generate takeaways - no
...
text and return it as JSON. If a field cannot be found, use an empty
string.
...
console.log('Gemini resume parsing failed, trying OpenAI fallback:',
geminiError?.message);
...
return res.status(500).json({ error: 'No AI provider available for
resume parsing' });
...
const dbType = process.env.SUPABASE_DATABASE_URL ? 'Supabase' :
'Replit';
...
console.log('Media storage: Database (base64) - Object Storage not
available');
